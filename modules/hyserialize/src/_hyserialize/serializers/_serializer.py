...
from _hycore.typefunc import Offset, get_subclasses_recursion, get_type_name
...
def dumps(obj, serializer: AbstractSerializer = None):
    """
    序列化数据
    :param obj: 需要序列化的对象
...
def loads(data: bytes, serializer: AbstractSerializer = None, ignore=False, use_my_serializer=False):
    """
    从字节串反序列化数据
    :param data: 字节串
    :param serializer: 指定一个序列化器，如果为 None，则使用 Json
    ...
    subclasses = {
        t.__name__: t
        for t in get_subclasses_recursion(AbstractSerializer)
    }
