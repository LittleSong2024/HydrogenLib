...
type Pointer[T] = T | Pointer[T] | PointerType | int  | None  # 正常来说, hyctypes 会执行自动转换为指针类型
type Ref[T] = T | Ref[T] | RefType | None
...
def parse_amonymous(tp):
    return typing.get_args(tp)[0]


def as_ctype(tp):
    if is_alias_type(tp):
        return py_to_c_mapping[tp]  # 如果只是直接的别名，那么直接返回对应的类型

    elif is_generic_alias_type(tp):  # 否则需要处理泛型
        origin = typing.get_origin(tp)
        match origin:
            case otp if otp is Pointer:
                ptr_type = parse_pointer(tp)
                return ctypes.POINTER(as_ctype(ptr_type))  # 注意对类型进行二次转换

            case opt if opt is Array:
                array_type, array_len = parse_array(tp)
                return as_ctype(array_type) * array_len  # Ctypes array 的创建方式是乘法

            case opt if opt is Anonymous:
                anonymous_type = parse_amonymous(tp)
                return as_ctype(anonymous_type)

            case opt if opt is Ref:
                target_type = parse_ref(tp)
                return ctypes.POINTER(as_ctype(target_type))

    elif issubclass(tp, typefunc.builtin_types):
        return py_to_c_mapping[tp]

    return None


if __name__ == '__main__':  # Test for type hints
    ctype = as_ctype(Array[Pointer[longlong], 100])
    print(ctype)
